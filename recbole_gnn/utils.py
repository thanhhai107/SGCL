...
from recbole.data.utils import load_split_dataloaders, create_samplers, save_split_dataloaders #, get_dataloader
from recbole.data.utils import create_dataset as create_recbole_dataset
from recbole.data.dataloader import TrainDataLoader, FullSortEvalDataLoader, NegSampleEvalDataLoader
...
from recbole_gnn.data.dataloader import MixTrainDataLoader, CustomizedTrainDataLoader, CustomizedNegSampleEvalDataLoader, CustomizedFullSortEvalDataLoader
from recbole.utils import ModelType as MT
...
def recbole_data_preparation(config, dataset):
...
def get_dataloader(config, phase: Literal["train", "valid", "test", "evaluation"]):
    ...
    if phase == "train":
        if model_type != MT.KNOWLEDGE:
            if config["model"] == 'MixSGCL':
                return MixTrainDataLoader
            else:
