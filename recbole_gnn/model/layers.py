...
class GINConv(MessagePassing):
    r"""Propagate a layer of GNN

    .. math::
        output MLP for message passing
    """
    def __init__(self, size_list):
        super().__init__(aggr='add')
        self.size_list = size_list    
        self.mlp = MLPLayers(self.size_list)
        
    def forward(self, x, edge_index, edge_weight):
        x_prop = self.propagate(edge_index, x=x, edge_weight=edge_weight)
        x_trans = self.mlp(x_prop)
        return x_trans 
    ...
    def __repr__(self):
        return '{}({})'.format(self.__class__.__name__, self.size_list)
